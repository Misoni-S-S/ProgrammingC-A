...
async function getAnswer(size,id){//id=>数字, 問題を取りだす。問題はtargetAnswer.boardで読み込めるよ
    ...
    try{
        ...
        if (targetAnswer) {
            ...
            answer_title = targetAnswer.title;
...
function getSizeCategoryIndex(size) {
    const sizeOrder = [3, 5, 10]; // answer.json の並び順と合わせる
...
// idInput の選択肢を、選ばれているサイズの問題数に合わせて更新
async function updateIdOptions() {
    const sizeInput = document.getElementById("sizeInput");
    const idInput = document.getElementById("idInput");
    const selectedSize = parseInt(sizeInput.value, 10);
    ...
    try {
        const response = await fetch('answer.json');
        const answerList = await response.json();
        const answerObject = answerList.find(t => t.size === selectedSize);
        const issues = answerObject ? answerObject.issue : [];
        const rowIndex = getSizeCategoryIndex(selectedSize);
...
//<button onclick="flip_setting_item(event); updateIdOptions();">
//    問題選択に戻る
//</button>

document.getElementById("sizeInput").addEventListener("change", updateIdOptions);
updateIdOptions(); // 初回読み込み時にも実行